...
export const copy = {
  ...
  kit: {
    verify: {
      ...
      /* Teto honesto (engines#650 Q2): nível 2 = assinatura técnica verificável, não validade legal. */
      headlineTail: { 3: "em seu nome, com titularidade verificada.", 2: "em seu nome, com assinatura verificável.", 1: "em seu nome." },
      ...
      checkDetail: {
        ...
        hashFail: "o conteúdo não bate com o registro",
        hashAlg: (alg: string) => `algoritmo desconhecido: ${alg}`,
        anchor: (n: number, data: string) => (n === 1 ? `1 registro ancorado na rede` : `${n} registros ancorados na rede`) + (data ? ` · ${data}` : ""),
        anchorFail: (ok: number, total: number) => `${ok} de ${total} registros com âncora confirmada`,
        sig: (quem: string) => `${quem} · assinatura Ed25519 conferida no navegador`,
        sigDefarm: "chave operada pela plataforma · irretratabilidade limitada",
        sigFail: (ok: number, bad: number) => bad > 0 ? `${bad} assinaturas não conferem` : ok === 0 ? "nenhum registro assinado" : "assinatura não conferida",
        revoke: "não foi revogada · conferido agora",
        revokeFail: "cortada por quem emitiu",
        revokeUnknown: "não foi possível conferir agora",
      },
      ...
      hiddenBodyCount: (n: number) => `${n} campos existem no registro original, mas foram selados antes da emissão. Nem você, nem a DeFarm.`,
